...
class AluguelController{
    static async criar(req, res){
        try {
            ...
            const novoAluguel = await aluguelModel.criar(nome, data_inicio, data_termino, valor, status)
    ...
    static async editar(req, res){
        try {
            ...
            const editAluguel = await aluguelModel.editar(id, data_termino, status)
    ...
    static async listarPorID(req, res){
        try {
            const id = req.params.id;
            const listar = await aluguelModel.listarPorID(id)
            if(listar.length === 0){
                return res.status(400).json({ msg: "Aluguel não encontrado" })
            }
    ...
    static async listarAll(req, res){
        try {
            const consulta = await aluguelModel.listarAll()
    ...
    static async deletarPorID(req, res){
        try {
            const id = req.params.id;
            const deleta = await aluguelModel.deletarPorID(id)
            if(deleta.length === 0){
                return res.status(400).json({ msg: "Aluguel não encontrado" })
            }
